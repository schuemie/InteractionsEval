...
evaluate2dPade <- function(dataSet, folder) {
  ...
  for (i in seq_along(outcomeIds)) {
    ...
    PadeEstCR(ebar,PadeCoef)
    
    # table(ppdata$siteID, ppdata$y > 0)
    
    
    save(ResPool, ResMeta, deriv, LocalDeriv, PadeCoef, file = file.path(folder, sprintf("PadeArtifacts_o%d.rdata", outcomeIds[i])))
    # 
    # e <- new.env()
